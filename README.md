# Growth-Skill 用户增长知识库

> 75个经典增长案例 | 12大增长流派 | 111种实战玩法 | 系统方法论

[![GitHub stars](https://img.shields.io/github/stars/tinylion1024/growth-skill)](https://github.com/tinylion1024/growth-skill/stargazers)
[![License](https://img.shields.io/github/license/tinylion1024/growth-skill)](https://github.com/tinylion1024/growth-skill/blob/main/LICENSE)
[![Cases](https://img.shields.io/badge/Cases-75-blue)](./cases/)
[![Growth Schools](https://img.shields.io/badge/Growth%20Schools-12-green)](./growth-schools/)
[![Weapons](https://img.shields.io/badge/Weapons-111-orange)](./weapons/)

---

## 项目简介

Growth-Skill 是一个专注**用户增长**领域的开源知识库，涵盖：

- **75个实战案例** — 涵盖中国、海外、垂直行业三大板块
- **12大增长流派** — 从增长黑客到品牌驱动，系统梳理增长方法论
- **111种增长玩法** — 冷启动、病毒裂变、内容增长、PLG 等可落地战术
- **7大核心模块** — 战略、数据、技术全方位体系

整理了拼多多、抖音、SHEIN、Notion、Airbnb 等国内外顶级互联网公司的增长实践。

---

## 快速导航

### 🎯 按场景

| 场景 | 案例数 | 代表案例 |
|------|--------|----------|
| [电商零售](./cases/) | 15+ | 拼多多、抖音电商、小红书、SHEIN |
| [社交娱乐](./cases/) | 12+ | 微信红包、B站、Soul、字节系 |
| [金融科技](./cases/) | 8+ | 支付宝、蚂蚁森林、微信支付 |
| [AI产品](./cases/) | 15+ | ChatGPT、Midjourney、Notion、Cursor |
| [SaaS工具](./cases/) | 10+ | Zoom、Slack、HubSpot、Calendly |
| [双边市场](./cases/) | 8+ | 滴滴、Uber、Airbnb、美团 |

### 📚 按流派

| 流派 | 核心玩法 |
|------|----------|
| [增长黑客派](./growth-schools/01-growth-hacking/) | A/B测试、数据实验、MVP验证 |
| [产品驱动增长](./growth-schools/02-product-led-growth/) | Freemium、Onboarding、自传播 |
| [网络效应](./growth-schools/03-network-effects/) | 双边市场、病毒传播 |
| [内容增长](./growth-schools/04-content-growth/) | SEO、入站营销、UGC |
| [社区增长](./growth-schools/05-community-growth/) | 社区运营、超级用户 |
| [品牌驱动](./growth-schools/06-brand-growth/) | 品牌故事、价值观营销 |

### 🛠️ 按武器

| 模块 | 玩法数 | 适用阶段 |
|------|--------|----------|
| [冷启动增长](./weapons/01-cold-start/) | 10 | 0→1 |
| [病毒裂变](./weapons/02-viral-referral/) | 15 | 所有阶段 |
| [内容增长](./weapons/03-content-growth/) | 15 | 0→10 |
| [社区增长](./weapons/04-community/) | 10 | 1→10 |
| [PLG](./weapons/05-plg/) | 15 | SaaS必备 |
| [留存增长](./weapons/06-retention/) | 10 | 所有阶段 |

---

## 目录结构

```
growth-skill/
├── cases/                      # 75个增长案例
│   ├── china/                  # 中国案例（38个）
│   │   ├── alipay.md          # 支付宝
│   │   ├── pinduoduo-group-buy.md
│   │   ├── douyin-algorithm.md
│   │   ├── xiaohongshu.md
│   │   └── ...
│   ├── overseas/              # 海外案例（27个）
│   │   ├── notion.md
│   │   ├── airbnb.md
│   │   ├── slack.md
│   │   └── ...
│   └── vertical/              # 垂直行业（10个）
│       ├── shein.md
│       ├── tiktok.md
│       └── ...
│
├── growth-schools/            # 12大增长流派
│   ├── 01-growth-hacking/      # 增长黑客派
│   ├── 02-product-led-growth/   # 产品驱动增长
│   ├── 03-network-effects/      # 网络效应增长
│   ├── 04-content-growth/        # 内容增长
│   ├── 05-community-growth/      # 社区增长
│   ├── 06-brand-growth/          # 品牌驱动增长
│   ├── 07-viral-growth/          # 病毒传播增长
│   ├── 08-performance-marketing/ # 付费广告增长
│   ├── 09-gamification/           # 游戏化增长
│   ├── 10-flywheel-growth/        # 增长飞轮
│   ├── 11-business-models/       # 商业模型
│   └── 12-growthhackers/         # GrowthHackers体系
│
├── weapons/                    # 增长武器库（111种玩法）
│   ├── 01-cold-start/          # 冷启动
│   ├── 02-viral-referral/      # 病毒裂变
│   ├── 03-content-growth/       # 内容增长
│   ├── 04-community/           # 社区增长
│   ├── 05-plg/                 # 产品驱动增长
│   ├── 06-retention/           # 留存增长
│   ├── 07-monetization/        # 变现增长
│   ├── 08-paid-ads/            # 付费广告
│   ├── 09-brand/               # 品牌增长
│   └── 10-b2b-sales/           # B2B销售
│
├── guides/                     # 核心方法论（5篇）
│   ├── aida-model.md          # AIDA转化模型
│   ├── growth-loop.md         # 增长飞轮
│   ├── north-star-metric.md   # 北极星指标
│   ├── programmatic-ads.md   # 程序化广告
│   └── user-profile.md        # 用户画像
│
├── modules/                    # 系统学习模块（7篇）
│   ├── module-01-overview.md  # 增长概述
│   ├── module-02-strategy.md  # 增长战略
│   ├── module-03-lean-growth.md # 精益增长
│   ├── module-04-research.md   # 用户研究
│   ├── module-05-acquisition.md # 获客策略
│   ├── module-06-retention.md  # 留存策略
│   └── module-07-data-tech.md  # 数据与技术
│
└── README.md
```

---

## 热门案例

### 中国案例

| 案例 | 标签 | 亮点 |
|------|------|------|
| [拼多多](./cases/china/pinduoduo-group-buy.md) | 拼团、社交裂变 | 微信生态下的社交电商 |
| [抖音算法](./cases/china/douyin-algorithm.md) | 推荐算法、内容分发 | 个性化推荐驱动增长 |
| [小红书](./cases/china/xiaohongshu.md) | 种草、KOC、社区 | KOC种草社区模式 |
| [微信红包](./cases/china/wechat-redpacket.md) | 红包、绑定银行卡 | 春晚红包裂变 |
| [蚂蚁森林](./cases/china/ant-forest.md) | 游戏化、公益 | 低碳行为游戏化 |
| [趣头条](./cases/china/qutoutiao.md) | 信息流、奖励、师徒体系 | 师徒裂变+积分体系 |
| [桌面Widget增长](./cases/china/widget-growth.md) | Widget, 社交货币, 高频曝光 | TikTok种草+锁屏曝光双轮驱动 |
| [白拿](./cases/china/white-get-gift.md) | 现金膨胀, 邀请裂变, 信息差 | 老用户召回+阶梯奖励 |

### 海外案例

| 案例 | 标签 | 亮点 |
|------|------|------|
| [Notion](./cases/overseas/notion.md) | PLG、社区、模板 | 社区驱动的产品增长 |
| [Airbnb](./cases/overseas/airbnb.md) | 信任机制、体验 | 房东+房客双边市场 |
| [Slack](./cases/overseas/slack.md) | 协作工具、邀请 | 病毒传播的典范 |
| [Dropbox](./cases/overseas/dropbox.md) | 邀请裂变、免费 | 邀请双方得奖励 |
| [Allbirds](./cases/overseas/allbirds.md) | DTC、口碑 | 环保品牌的口碑增长 |

### 垂直行业

| 案例 | 行业 | 亮点 |
|------|------|------|
| [SHEIN](./cases/vertical/shein.md) | 快时尚 | 供应链+DTC出海 |
| [TikTok](./cases/vertical/tiktok.md) | 短视频 | 算法推荐全球化 |
| [Duolingo](./cases/vertical/duolingo.md) | 教育 | 游戏化学习 |
| [Coursera](./cases/vertical/coursera.md) | MOOC | 订阅+认证模式 |

---

## 增长公式

```
增长 = 获客 × 激活 × 留存 × 变现 × 传播
```

| 阶段 | 核心问题 | 关键指标 |
|------|----------|----------|
| 获客 | 如何低成本获取用户？ | CAC、转化率 |
| 激活 | 用户快速体验到价值？ | 激活率、时间成本 |
| 留存 | 如何让用户持续使用？ | 留存率、流失率 |
| 变现 | 如何从用户身上赚钱？ | LTV、ARPU |
| 传播 | 如何让用户主动推荐？ | K因子、NPS |

---

## 学习路径

```
1️⃣ 入门 → 阅读 modules/01-overview.md 了解增长框架
     ↓
2️⃣ 方法论 → 学习 guides/ 中的核心模型（AIDA、增长飞轮、北极星指标）
     ↓
3️⃣ 流派 → 浏览 growth-schools/ 选择适合自己的增长方向
     ↓
4️⃣ 武器 → 在 weapons/ 中寻找可落地的具体玩法
     ↓
5️⃣ 实战 → 通过 cases/ 学习具体案例，按行业/标签筛选
     ↓
6️⃣ 复盘 → 应用到自己业务，验证并迭代
```

---

## 案例结构

每个案例包含完整的9个章节：

| 章节 | 说明 |
|------|------|
| 完整背景 | 公司/产品/行业/阶段 |
| 核心挑战 | 主要增长问题 |
| 增长策略 | 具体方法与执行 |
| 关键数据 | 核心指标与成果 |
| 核心洞察 | 深层规律总结 |
| 可复制点 | 可落地框架 |
| 常见误区 | 需避免的错误 |
| 关键成功因素 | 核心成功要素 |
| 后续演进 | 发展与未来方向 |

---

## 贡献

欢迎提交 Issue 或 PR！

- 提交新案例（请参考现有案例格式）
- 补充缺失的增长玩法
- 完善方法论文档
- 修正内容错误

---

**GitHub**: https://github.com/tinylion1024/growth-skill

**最后更新**：2026年4月
